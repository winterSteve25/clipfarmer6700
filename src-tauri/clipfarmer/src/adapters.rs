...
impl SignalExtractor for FfmpegSignalExtractor {
    async fn extract(&self, input_path: &str, start_ms: i64, end_ms: i64) -> Result<LocalSignals> {
        ...
        let window_seconds = (end_ms - start_ms) as f64 / 1_000.0;
        // ffmpeg 7+ exits non-zero ("Could not open encoder before EOF") when a filtered output
        // never receives a frame, e.g. `select` drops every frame of a static window, or a corrupt
        // video packet leaves nothing to decode. volumedetect has already printed by then, so a
        // failure is only fatal when no audio statistics were produced.
        match parse_signal_diagnostics(&diagnostics, window_seconds) {
            Some(signals) => {
                if !result.status.success() {
                    progress::warning(format!(
                        "ffmpeg exited with {} after measuring signals; using the partial result",
                        result.status
                    ));
                }
                Ok(signals)
            }
            None => anyhow::bail!("ffmpeg signal extraction failed: {diagnostics}"),
        }
    }
}

/// Parses `volumedetect` and `showinfo` output. Returns `None` when no mean volume was reported,
/// which means the audio branch never ran and the measurement cannot be trusted.
fn parse_signal_diagnostics(diagnostics: &str, window_seconds: f64) -> Option<LocalSignals> {
    let mean_db = diagnostics
        .lines()
        .find_map(|line| line.split("mean_volume:").nth(1))
        .and_then(|value| value.trim().split(' ').next())
        .and_then(|value| value.parse::<f64>().ok())?;
    // The filter always passes frame 0 (see SIGNAL_FILTER), so it is not a scene change.
    let selected_frames = diagnostics
        .lines()
        .filter(|line| line.contains("showinfo") && line.contains(" n:"))
        .count();
    let scene_changes = selected_frames.saturating_sub(1);
    Some(LocalSignals {
        audio_energy: ((mean_db + 60.0) / 60.0).clamp(0.0, 1.0),
        scene_change_rate: scene_changes as f64 / window_seconds.max(0.001),
    })
}

/// `eq(n,0)` guarantees at least one frame reaches the output. Without it, a window with no scene
/// change produces zero frames and newer ffmpeg builds fail the whole command.
const SIGNAL_FILTER: &str = "[0:a]volumedetect[a];[0:v]select='eq(n,0)+gt(scene,0.25)',showinfo[v]";
...
impl TwitchVodSource {
    ...
    pub async fn prepare_with_progress(
        ...
        progress::info(format!("Cache: {}", cache_dir.display()));
        ...
        let channel_step = Step::start("Resolving Twitch channel");
        let resolved_channel = match cached_channel {
            Some(channel) => {
                channel_step.skipped(format!("cached channel {channel}"));
                channel
            }
            None => {
                let channel = self.resolve_channel(&canonical_url, &id).await?;
                channel_step.done(channel.clone());
                channel
            }
        };
...
mod tests {
    ...
    #[test]
    fn signal_parsing_survives_a_failed_video_branch() {
        // Abridged ffmpeg 9 output for a window where the video filter emitted no frames.
        let log = "[Parsed_volumedetect_0 @ 0x1] n_samples: 575008\n\
                   [Parsed_volumedetect_0 @ 0x1] mean_volume: -27.3 dB\n\
                   [Parsed_volumedetect_0 @ 0x1] max_volume: -9.5 dB\n\
                   [vost#0:1/wrapped_avframe @ 0x2] Could not open encoder before EOF\n";
        let signals = parse_signal_diagnostics(log, 12.0).unwrap();
        assert_eq!(signals.scene_change_rate, 0.0);
        assert!((signals.audio_energy - (32.7 / 60.0)).abs() < 1e-9);
    }

    #[test]
    fn signal_parsing_does_not_count_the_forced_first_frame() {
        let log = "[Parsed_showinfo_2 @ 0x1] n:   0 pts: 0\n\
                   [Parsed_showinfo_2 @ 0x1] n:   1 pts: 3000\n\
                   [Parsed_showinfo_2 @ 0x1] config in time_base: 1/90000\n\
                   [Parsed_volumedetect_0 @ 0x1] mean_volume: -30.0 dB\n";
        let signals = parse_signal_diagnostics(log, 10.0).unwrap();
        assert_eq!(signals.scene_change_rate, 0.1);
    }
